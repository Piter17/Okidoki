...
typedef TypedInvocationFunc<T> = void Function(T param);

typedef TypedInvocationFunc2<T, T2> = void Function(T param, T2 param2);

typedef TypedInvocationFunc3<T, T2, T3> = void Function(
  T param,
  T2 param2,
  T3 param3,
);
...
class TypedMethodDefinition({
  ...
  required final void Function(List<Object?>? params) caller,
});

//    : _type = T {
//   final Type _type;
//   // final TypedInvocationFunc<T> _method;
//   // final void Function(dynamic, List<Object?>? args) _caller;

//   // TypedMethodDefinition()  : _method = method,
//   //       _caller = caller,
//   //       _type = T;
// }
...
class HubConnection {
  ...
  HubConnection(
    ...
    _hubConnectionStateMaintainer = _HubConnectionStateMaintainer(
  ...
  StreamController<Object?> streamControllable(
    ...
    final StreamController streamController = StreamController<Object?>(
      onCancel: () {
        final cancelInvocation = _createCancelInvocation(
  ...
  Future<Object?> invoke(String methodName, {List<Object>? args}) {
    ...
    _callbacks[invocationDescriptor
        .invocationId] = (HubMessageBase? invocationEvent, Exception? error) {
      if (error != null) {
  ...
  void _resetKeepAliveInterval() {
    ...
    _pingServerTimer = Timer.periodic(
      Duration(milliseconds: keepAliveIntervalInMilliseconds),
      (Timer t) async {
  ...
  _reconnect({Exception? error}) async {
    ...
    while (nextRetryDelay != null) {
      ...
      try {
        ...
        try {
          _reconnectedCallbacks.forEach(
            (c) => c(connectionId: _connection.connectionId),
          );
        } catch (e) {
          _logger?.severe(
  ...
  _launchStreams(
    Map<String, Stream<Object>> streams,
